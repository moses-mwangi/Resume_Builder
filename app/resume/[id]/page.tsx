...
//   //   // Load resume data (you can replace this with your actual data fetching)
//   //   useEffect(() => {
//   //     // Simulate loading resume data
//   //     const loadResume = async () => {
//   //       setIsLoading(true);
//   //       // TODO: Fetch from your backend or localStorage
...
export default function ResumeBuilder() {
  ...
  useEffect(() => {
    const loadResume = async () => {
      setIsLoading(true);
      try {
        // Try to load from resumes array
        const savedResumes = localStorage.getItem("resumeDataArray");
        if (savedResumes) {
          const resumesArray = JSON.parse(savedResumes);
          // Find resume with matching ID (handle both string and number IDs)
          const foundResume = resumesArray.find(
            (r: any) => r.id?.toString() === id || r.id === parseInt(id),
          );

          if (foundResume) {
            setResumeData(foundResume);
            setHistory([foundResume]);
            setHistoryIndex(0);
          } else {
            // Try individual resume storage
            ...
            if (individualResume) {
              ...
              // No resume found, create new one with the ID
              const newResume = {
                ...initialResumeData,
                id: id,
                title: `Resume ${id}`,
          ...
          // No resumes array exists, create new resume
          const newResume = {
            ...initialResumeData,
            id: id,
            title: `Resume ${id}`,
  ...
  useEffect(() => {
    if (!isLoading && resumeData.id) {
      const saveTimeout = setTimeout(() => {
        ...
        const existingIndex = resumesArray.findIndex(
          (r: any) => r.id?.toString() === resumeData.id?.toString(),
        );
